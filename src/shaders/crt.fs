// found at: https://www.shadertoy.com/view/Ms23DR
...
void main(void)
{
    vec2 q = gl_FragCoord.xy / uScreenSize;
    vec2 uv = q;
    uv = curve( uv );
    vec3 oricol = texture2D( uSampler, vec2(q.x,q.y) ).xyz;
    vec3 col;
	float x =  sin(0.3*uGlobalTime+uv.y*21.0)*sin(0.7*uGlobalTime+uv.y*29.0)*sin(0.3+0.33*uGlobalTime+uv.y*31.0)*0.0017;
	...
	float scans = clamp( 0.35+0.35*sin(3.5*uGlobalTime+uv.y*uScreenSize.y*1.5), 0.0, 1.0);

	float s = pow(scans,1.7);
	col = col*vec3( 0.4+0.7*s) ;

    col *= 1.0+0.01*sin(110.0*uGlobalTime);
    ...
    float comp = smoothstep( 0.1, 0.9, sin(uGlobalTime) );
